...
def build_target(preds, targets, cfg, device):
    ...
    gain = torch.ones(8, device = device)
    ...
    return tcls, tang, tbox, indices, anch
...
def compute_loss(preds, targets, cfg, device):
    ...
    lcls, lang, lbox, lobj = ft([0]), ft([0]), ft([0]), ft([0])

    #定义obj和cls的损失函数
    BCEcls = nn.CrossEntropyLoss() 
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(1.0, device=device))
    BCEang = nn.SmoothL1Loss()
    
    cp, cn = smooth_BCE(eps=0.0)
    
    #构建gt
    tcls, tang, tbox, indices, anchors = build_target(preds, targets, cfg, device)
    ...
    for i, pred in enumerate(preds):
        ...
        #计算cls分支loss
            ...
            # pred.shape = [batch, 2, height, width]
            pred = pred.reshape(pred.shape[0], 1, 2, pred.shape[2], pred.shape[3])
            pred = pred.permute(0, 1, 3, 4, 2)  # shape: B 1 H W 2
            ...
            if len(indices):
                ...
                if nb:
                    ps = pred[b, 0, gj, gi] # shape: B H W 2

                    if ps.size(1) > 1:
                        #t = torch.full_like(ps[:, :], cn)  # targets
                        #t[range(nb), tcls[layer_index[i]]] = cp
                        lang += BCEang(ps[:, :], tang[layer_index[i]]) / cfg["angle"]  # BCE
        else:
            print("error")
            raise
    ...
    lcls *= 32
    lang *= 32
    loss = lbox + lobj + lcls + lang

    return lbox, lobj, lcls, lang, loss
