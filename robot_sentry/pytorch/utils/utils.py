...
def load_datafile(data_path):
    ...
    int_type_key = ["epochs", "batch_size", "classes","angle", "width",
...
def non_max_suppression(prediction, conf_thres=0.3, iou_thres=0.45, classes=None):
    ...
    Returns:
         detections with shape: nx7 (x1, y1, x2, y2, conf, cls, angle)
    """
    ...
    t = time.time()
    output = [torch.zeros((0, 7), device="cpu")] * prediction.shape[0]
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        angle = torch.atan2(x[:, 12:13], x[:, 11:12])
...
def handel_preds(preds, cfg, device):
    ...
    anchors = torch.from_numpy(anchors.reshape(len(preds) // 4, cfg["anchor_num"], 2)).to(device)
    ...
    for i in range(len(preds) // 4):
        bacth_bboxes = []
        reg_preds = preds[i * 4]
        obj_preds = preds[(i * 4) + 1]
        cls_preds = preds[(i * 4) + 2]
        ang_preds = preds[(i* 4)+3]
        
        for r, o, c,angle in zip(reg_preds, obj_preds, cls_preds,ang_preds):
            r = r.permute(1, 2, 0)
            ...
            c = c.repeat(1, 1, 3, 1)
            angle = angle.permute(1, 2, 0)
            angle = angle.reshape(angle.shape[0],angle.shape[1], 1, angle.shape[2])
            angle = angle.repeat(1, 1, 3, 1)
            
            anchor_boxes = torch.zeros(r.shape[0], r.shape[1], r.shape[2], r.shape[3] + c.shape[3] + 1+angle.shape[3])
            ...
            #计算cls分数
            anchor_boxes[:, :, :, 5:11] = F.softmax(c[:, :, :, :], dim = 3)
            #计算angle
            anchor_boxes[:, :, :, 11:13] = angle[:, :, :, :]
...
def evaluation(val_dataloader, cfg, model, device, conf_thres = 0.01, nms_thresh = 0.4, iou_thres = 0.5):
    ...
    for imgs, targets in pbar:
        ...
        targets[:, 2:6] *= torch.tensor([cfg["width"], cfg["height"], cfg["width"], cfg["height"]]).to(device)
