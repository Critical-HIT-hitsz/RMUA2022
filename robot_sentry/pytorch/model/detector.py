...
class Detector(nn.Module):
    def __init__(self, classes, anchor_num, load_param, export_onnx = False):
        ...
        self.output_ang_layers = nn.Conv2d(out_depth, 2, 1, 1, 0, bias=True)

    def forward(self, x):
        C2, C3 = self.backbone(x)
        cls_2, obj_2, reg_2, ang_2, cls_3, obj_3, reg_3, ang_3 = self.fpn(C2, C3)
        
        out_reg_2 = self.output_reg_layers(reg_2)
        out_obj_2 = self.output_obj_layers(obj_2)
        out_cls_2 = self.output_cls_layers(cls_2)
        out_ang_2 = self.output_ang_layers(ang_2)

        out_reg_3 = self.output_reg_layers(reg_3)
        out_obj_3 = self.output_obj_layers(obj_3)
        out_cls_3 = self.output_cls_layers(cls_3)
        out_ang_3 = self.output_ang_layers(ang_3)
        
        if self.export_onnx:
            out_reg_2 = out_reg_2.sigmoid()
            out_obj_2 = out_obj_2.sigmoid()
            out_cls_2 = F.softmax(out_cls_2, dim = 1)
            out_ang_2 = F.tanh(out_ang_2)

            out_reg_3 = out_reg_3.sigmoid()
            out_obj_3 = out_obj_3.sigmoid()
            out_cls_3 = F.softmax(out_cls_3, dim = 1)
            out_ang_3 = F.tanh(out_ang_3)

            print("export onnx ...")
            return torch.cat((out_reg_2, out_obj_2, out_cls_2,out_ang_2), 1).permute(0, 2, 3, 1), \
                   torch.cat((out_reg_3, out_obj_3, out_cls_3,out_ang_3), 1).permute(0, 2, 3, 1)
                   

        else:
            return out_reg_2, out_obj_2, out_cls_2, out_ang_2, out_reg_3, out_obj_3, out_cls_3, out_ang_3
