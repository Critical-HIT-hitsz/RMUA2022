...
if __name__ == '__main__':
    ...
    model = model.detector.Detector(
        cfg["classes"], cfg["anchor_num"], load_param).cuda()
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for imgs, targets in pbar:
            ...
            preds = model(imgs)
            # loss计算
            iou_loss, obj_loss, cls_loss, lang_loss, total_loss = utils.loss.compute_loss(
            ...
            # 打印相关信息
            info = "Epoch:%d LR:%f CIou:%f Obj:%f Cls:%f Ang:%f Total:%f" % (
                epoch, lr, iou_loss, obj_loss, cls_loss, lang_loss, total_loss)
